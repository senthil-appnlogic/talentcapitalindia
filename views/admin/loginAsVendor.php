...
<div class="content" id="content">
			...
			{?>
			    ...
			    <?php
			     ...
			     <p>
				<a class="btn btn-primary btn-sm " href="<?php echo site_url('admin/vendorAddUser')?>"><i class="fa fa-plus fa-1x"></i> <span class="f-s-14 f-w-500">Add Candidate</span></a>
			    </p>
					...
					<table id="data-table" class="table table-striped table-bordered nowrap" width="100%">
					  <thead>
						...
						<tr class="oddClass even gradeC">
						    ...
						    <td id="USER_NAME" style="cursor:pointer;" class="clickable-row" data-href='<?php echo site_url('admin/vendorloginEditReadOnly/'.$row['id'])?>'><u><?php echo $row['name']; ?></u></td>					    
						    <td><?php echo $row['mobile_number']; ?></td>
						    <td id="EMAIL"><?php echo $row['email']; ?></td>
						    <td><?php echo $row['pan_attach_no']; ?></td>				    
						    <td><?php echo $row['address_text']; ?></td>
						 
						    <td>
						    
						    <a href="<?php echo site_url('admin/vendorloginEdit/'.$row['id'])?>" class="btn btn-info btn-xs"><i class="fa fa-edit"></i> </a>
						    </td>
						</tr>
					    <?php }}?>
					    </tbody>
	...
	$(document).ready(function() {
	 ...
	 setTimeout(function(){ $('#alert').remove();}, 5000);
		...
		{
		    var data = "admin/vendorPrint/"+systemId;
		    var preview = "<?php echo site_url()?>"+data;
		    $('#previewiframe').attr('src',preview);
		    var dataURL = "admin/vendorPrint/"+systemId;
