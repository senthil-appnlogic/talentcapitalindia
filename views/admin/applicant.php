...
<div class="content" id="content">
			...
			{?>
				...
				<p>
				   <a class="btn btn-primary btn-sm " href="<?php echo site_url('admin/applicantAdd')?>"><i class="fa fa-plus fa-1x"></i> <span class="f-s-14 f-w-500">Add Applicant</span></a>
				 </p>
					...
					<table id="data-table" class="table table-striped table-bordered nowrap" width="100%">
					  <thead>
						<tr>
						    ...
						    <th data-hide="phone,tablet">Skill</th>
					    ...
					    <tbody>
						<?php if(count($applicantDetails) > 0)
							     { foreach($applicantDetails as $row) {  ?>
					       
						<tr>
						    <td style="cursor:pointer;" class="clickable-row" data-href='<?php echo site_url('admin/applicantEditView/'.$row['id'])?>'><u><?php echo $row['name']; ?></u></td>					    
						    <td><?php echo $row['mobile_number']; ?></td>
						    <td><?php echo $row['email']; ?></td>
						    <td><?php echo $row['skill']; ?></td>				    
						
						    <td>
							<a href="<?php echo site_url('admin/applicantEdit/'.$row['id'])?>" class="btn btn-info btn-xs"><i class="fa fa-edit"></i> </a>
							<a id="delete_box" href="<?php echo site_url('admin/applicantDelete/'.$row['id'])?>" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> </a>
							<a  href="#" data-toggle="modal" data-target="#printPreview"  class="btn btn-primary btn-xs" onclick="printApplicantDetails('<?php echo $row['id'];?>')"><i class="fa fa-file-pdf-o fa-2x"></i> </a>
						    </td>
						</tr>
					    <?php }}?>
					    </tbody>
	...
	function printApplicantDetails(systemId)
		
		{
		    var data = "admin/applicantPrint/"+systemId;
		    var preview = "<?php echo site_url()?>"+data;
		    $('#previewiframe').attr('src',preview);
		    var dataURL = "admin/applicantPrint/"+systemId;
